...
SoundToy.prototype.newScript = function(url)
{
    ...
    {
        alert( "1could not load script " + url + ": " + e );
        return false;
    }
    var xml = this.mHttpReq.responseXML;
    if( xml==null )
    {
        alert( "2could not load script" );
        return false;
    }
    ...
    {
        alert( "3could not load script" );
        return false;
    }
    var version = ele_shadertoy[0].attributes["version"].value;
    if( version != "0.1" )
    {
        alert( "4could not load script" );
        return false;
    }
